    ...
    def register_handlers(self):
        ...
        async def process_phone(message: types.Message, state: FSMContext):
            ...
            try:
                ...
                authorized = await self.create_telethon_client(
                    user_data['api_id'],
                    user_data['api_hash'],
                    phone
                )
        ...
        async def process_code(message: types.Message, state: FSMContext):
            ...
            try:
                await self.telethon_client.sign_in(code=code)
                ...
                logger.error(f"Неизвестная ошибка при вводе кода: {e}")
                await message.answer(
                    "⚠️ Произошла ошибка. Попробуйте снова командой /start"
                )
                await state.clear()
