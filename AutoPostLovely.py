...
"""
{
    "tasks": {
        "task_id": {
            ...
            "interval": 300,
            "active": false,
            "last_posted": "2023-01-01T12:00:00"
        }
    },
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
                config = json.load(f)
                # Добавляем настройки уведомления, если их нет
...
# Обновленная функция запуска задачи с использованием семафора
async def run_task(task_id: str, task_data: Dict, bot: Bot) -> None:
    ...
    while task_id in active_tasks and active_tasks[task_id]:
        try:
            # Ждем доступа к семафору (очереди задач)
...
def get_main_menu_keyboard() -> InlineKeyboardMarkup:
    config = load_config()
    notifications_status = "🔔 Вкл." if config.get("notifications_enabled", True) else "🔕 Выкл."
...
async def start_tasks_in_batches(tasks: Dict[str, Dict], bot: Bot, batch_size: int = 5, delay: int = 30):
    """Запускает задачи по частям с задержкой между группами"""
    ...
    for batch_num, batch in enumerate(batches, 1):
        # Отправляем уведомление о запуске группы задач
        if owner_id and notifications_enabled:
            await bot.send_message(
                owner_id,
                f"🔄 Запуск группы задач {batch_num}/{len(batches)} (всего {len(batch)} задач)"
            )

        # Запускаем задачи в текущей группе
        for task_id in batch:
            if task_id in tasks:
                ...
                # Логируем запуск задачи
                logging.info(f"Запущена задача {task_id} для группы @{task_data['group_username']}")

        # Сохраняем обновленную конфигурацию
        save_config(config)

        # Если это не последняя группа, делаем задержку перед запуском следующей
        if batch_num < len(batches):
            await asyncio.sleep(delay)

    # Отправляем уведомление о завершении запуска всех задач
    if owner_id and notifications_enabled:
        await bot.send_message(
            owner_id,
            f"✅ Все задачи успешно запущены ({len(task_ids)} задач)"
        )
...
async def process_toggle_notifications(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    # Отправляем сообщение с обновленным статусом
...
async def main():
    ...
    # Обработчик кнопки "Включить все"
    ...
    async def process_start_all_tasks(callback_query: types.CallbackQuery, state: FSMContext):
        ...
        if not tasks:
            await bot.send_message(
                callback_query.from_user.id,
                "У вас пока нет задач. Добавьте новые задачи с помощью кнопки '➕ Добавить задачу'.",
                reply_markup=get_main_menu_keyboard()
            )
            return

        # Фильтруем только неактивные задачи
        inactive_tasks = {task_id: task_data for task_id, task_data in tasks.items()
        ...
        # Отправляем сообщение о начале запуска задач
        await bot.send_message(
            callback_query.from_user.id,
            f"🔄 Начинаю запуск задач по группам (по 5 задач с интервалом 30 секунд)...\n"
            f"Всего будет запущено {len(inactive_tasks)} задач.",
            reply_markup=get_main_menu_keyboard()
        )

        # Запускаем задачи по частям
        asyncio.create_task(start_tasks_in_batches(inactive_tasks, bot))
    ...
    if active_tasks_data:
        logging.info(f"Запуск {len(active_tasks_data)} активных задач по группам...")
        asyncio.create_task(start_tasks_in_batches(active_tasks_data, bot))
