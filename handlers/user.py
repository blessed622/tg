...
# ИСПРАВЛЕННАЯ версия с выбором топика
...
# Новый обработчик для выбора топика
    ...
    try:
        ...
        if success:
            ...
            await message.answer(
                ...
                f"Интервал: {interval_text}\n"
                f"Текст: {message_text[:100]}..." if len(message_text) > 100 else message_text
            )
        else:
            await message.answer(
...
async def buy_subscription(callback: types.CallbackQuery):
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="💳 Оплатить", url="https://t.me/https://t.me/lovelymaxing")],
